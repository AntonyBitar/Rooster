...
class CreateNewSupplierOrder extends StatefulWidget {
  const CreateNewSupplierOrder({super.key});

  @override
  State<CreateNewSupplierOrder> createState() => _CreateNewSupplierOrderState();
}

class _CreateNewSupplierOrderState extends State<CreateNewSupplierOrder> {
  ...
  // final SupplierOrderController supplierOrderController = Get.find();
  final QuotationsController quotationController = Get.find();
  final HomeController homeController = Get.find();
  int progressVar = 0;
  Map data = {};
  // bool isOrdersInfoFetched = false;
  bool isSupplierOrdersInfoFetched = false;
  ...
  getFieldsForCreateQuotationFromBack() async {
    ...
    // var p = await getFieldsForCreateSupplierOrder();
    var p = await getFieldsForCreateQuotation();
    supplierOrderNumber = p['quotationNumber'];
    if ('$p' != '[]') {
      setState(() {
        data.addAll(p);
        supplierOrderNumber = p['quotationNumber'] as String;
        for (var client in p['clients']) {
          customerNameList.add('${client['name']}');
          customerIdsList.add('${client['id']}');
        }
        isSupplierOrdersInfoFetched = true;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return isSupplierOrdersInfoFetched
        ? Container(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            // isSupplierOrdersInfoFetched
                            //     ? Text(
                            //     supplierOrderNumber, //'${data['supplierOrderNumber'].toString() ?? ''}',
                            //     style: TextStyle(
                            //         fontWeight: FontWeight.bold,
                            //         fontSize: 20,
                            //         color:
                      ...
                      onTapFunction: () async {
                        ...
                        if (res['success'] == true) {
                          CommonWidgets.snackBar('Success',
                              res['message']);
                          setState(() {
                            isSupplierOrdersInfoFetched = false;
                            getFieldsForCreateQuotationFromBack();
                          });
...
class MobileCreateNewSupplierOrder extends StatefulWidget {
  const MobileCreateNewSupplierOrder({super.key});

  @override
  State<MobileCreateNewSupplierOrder> createState() =>
      _MobileCreateNewSupplierOrderState();
}

class _MobileCreateNewSupplierOrderState extends State<MobileCreateNewSupplierOrder> {
  // bool imageAvailable=false;
  ...
  getFieldsForCreateQuotationFromBack() async {
    ...
    var p = await getFieldsForCreateQuotation();
    supplierOrderNumber = p['supplierOrderNumber'];
    if ('$p' != '[]') {
      setState(() {
        data.addAll(p);
        supplierOrderNumber = p['supplierOrderNumber'] as String;
        for (var client in p['clients']) {
          customerNameList.add('${client['name']}');
          customerIdsList.add('${client['id']}');
        }
