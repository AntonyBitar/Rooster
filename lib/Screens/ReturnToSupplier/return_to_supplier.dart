...
class CreateNewReturnToSupplier extends StatefulWidget {
  const CreateNewReturnToSupplier({super.key});

  @override
  State<CreateNewReturnToSupplier> createState() => _CreateNewReturnToSupplierState();
}

class _CreateNewReturnToSupplierState extends State<CreateNewReturnToSupplier> {
  ...
  // final ReturnToSupplierController returntosupplierController = Get.find();
  final QuotationsController quotationController = Get.find();
  final HomeController homeController = Get.find();
  int progressVar = 0;
  Map data = {};
  // bool isOrdersInfoFetched = false;
  bool isReturnToSuppliersInfoFetched = false;
  ...
  getFieldsForCreateQuotationFromBack() async {
    ...
    // var p = await getFieldsForCreateReturnToSupplier();
    ...
    if ('$p' != '[]') {
      setState(() {
        ...
        isReturnToSuppliersInfoFetched = true;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return isReturnToSuppliersInfoFetched
        ? Container(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            // isReturnToSuppliersInfoFetched
                            //     ? Text(
                      ...
                      onTapFunction: () async {
                        ...
                        if (res['success'] == true) {
                          CommonWidgets.snackBar('Success',
                              res['message']);
                          setState(() {
                            isReturnToSuppliersInfoFetched = false;
                            getFieldsForCreateQuotationFromBack();
                          });
...
class MobileCreateNewReturnToSupplier extends StatefulWidget {
  const MobileCreateNewReturnToSupplier({super.key});

  @override
  State<MobileCreateNewReturnToSupplier> createState() =>
      _MobileCreateNewReturnToSupplierState();
}

class _MobileCreateNewReturnToSupplierState extends State<MobileCreateNewReturnToSupplier> {
  // bool imageAvailable=false;
